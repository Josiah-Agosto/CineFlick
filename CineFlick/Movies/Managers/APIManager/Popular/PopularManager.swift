...
final class PopularManager {
    ...
    public func createPopularPosterUrls() {
        manager.popularFilePaths.forEach({ (paths) in
            self.manager.popularImagesURLs.append("\(self.manager.configurationManager.secureBaseUrl)\(self.manager.configurationManager.imageSize)\(paths)")
        })
    }
    ...
    public func createPopularBackdropUrls() {
        manager.popularBackdropPaths.forEach({ (backdrops) in
            self.manager.popularBackdropURLs.append("\(self.manager.configurationManager.secureBaseUrl)\(self.manager.configurationManager.backdropSize)\(backdrops)")
        })
    }
